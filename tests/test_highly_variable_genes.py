...
def test_get_gene_means_variances_sparse(test_adata_sparse):
    ...
    expected_variances = np.array([21742965.33455364, 31907224.71032163,  7862088.7369532,
                    22253503.67957076, 83184369.2786943, 1648403.15901494,
                    25237.22612277, 157585.43876406, 13712.69209709,
                    112416.51866621, 155656.27505236, 9047576.81752231])


    # test means_vars_genes
    adata_sparse = test_adata_sparse
    means, variances, gene_mask = tc.means_vars_genes(adata = adata_sparse, chunk_size = 5)
    np.testing.assert_allclose(
        means,
        expected_means,
        rtol=1e-06,
...
def test_get_gene_means_variances_dense(test_adata_dense):
    ...
    means, variances, gene_mask = tc.means_vars_genes(adata = adata_dense, chunk_size = 5)
    np.testing.assert_allclose(
        means,
        expected_means,
        rtol=1e-06,
